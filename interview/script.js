...
function getPostByUserID (posts, userId) 
{
    return posts.filter(posts => posts["userId"] === userId);
}
    console.log(getPostByUserID(posts, 1));

function getPostLength30(posts) 
{
    return posts.filter(posts => posts["title"].length < 30);
}
console.log(getPostLength30(posts));
function checkPost200Length(posts) 
{
    return posts.filter(posts => posts["body"].length > 200)
}
function truncate(posts)
{
    posts = checkPost200Length(posts);
    for(let i = 0; i < posts.length; i++)
    {
        searchValue = posts[i].body.slice(200);
        posts[i].body = posts[i].body.replace(searchValue,"...");
    }
    return posts;
}
console.log(truncate(posts));